...
## 2026-06-29 — Finding: render-blocking CSS (PSI mobile) — Claude Code
- PSI mobile "Render-blocking requests" insight: the only blocking resource is `/dist/style.css` (6.8 KiB transfer, ~290 ms on throttled mobile); 0 third-party blocking (GTM/GA/Ads/Clarity are async). PSI *Insights* models up to ~1.24 s LCP headroom — theoretical: measured FCP 1.0 s / LCP 1.8 s are already green because the LCP poster is preloaded and doesn't wait on the CSS. Desktop = 98, not flagged (lab CPU/network throttling is why it's mobile-only).
- DECISION: **defer** — fix properly via inline-critical-CSS + non-blocking load **with the build step the SEO/expansion phase will introduce** (keep `dist/style.css` as the shared cached sheet for multi-page). No-build alternative (inline the whole 6.8 KiB into `<head>` — removes the block, no FOUC) was offered but declined to avoid churn before going multi-page. Captured in CLAUDE.md parked items.
