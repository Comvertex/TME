...
Also parked: CTA → Airtable intake form (TME OS Pilot 1); optional `.gitattributes` (`* text=auto eol=lf`) for CRLF. **Render-blocking CSS** — PSI mobile flags `/dist/style.css` (30KB/~6KB gz) for ~100ms (desktop 98, no flag — it's lab CPU/network throttling). Proper fix = inline critical CSS + load the rest non-blocking; do this **with the build step the expansion phase will introduce** (keep `dist/style.css` as the shared cached sheet — don't hand-async the whole file, it causes FOUC).
